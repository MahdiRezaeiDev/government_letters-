...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('letters', function (Blueprint $table) {
            ...
            $table->enum('priority', [
                'low', 'normal', 'high', 'urgent', 'very_urgent'
            ])->default('normal');
            ...
            // outgoing  → position_id
            // internal  → position_id
            $table->unsignedBigInteger('sender_id')->nullable();
            $table->string('sender_name')->nullable();       // کپی ثابت
            $table->string('sender_position')->nullable();   // کپی ثابت
            $table->foreignId('sender_department_id')
                ->nullable()
                ->constrained('departments')
                ->nullOnDelete();

            // ─── گیرنده ───
            // incoming  → position_id
            // outgoing  → organization_id
            // internal  → position_id
            $table->unsignedBigInteger('recipient_id')->nullable();
            $table->string('recipient_name')->nullable();    // کپی ثابت
            $table->string('recipient_position')->nullable();// کپی ثابت
            $table->foreignId('recipient_department_id')
                ->nullable()
                ->constrained('departments')
                ->nullOnDelete();
            ...
            $table->foreignId('is_response_to')
                ->nullable()->constrained('letters')->nullOnDelete();
            $table->foreignId('is_follow_up')
                ->nullable()->constrained('letters')->nullOnDelete();
            $table->integer('follow_up_count')->default(0);
            $table->integer('sheet_count')->nullable();
            
            $table->boolean('is_draft')->default(false);
            $table->foreignId('draft_of')
                ->nullable()->constrained('letters')->nullOnDelete();
