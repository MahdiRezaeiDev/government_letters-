...
export default function UsersIndex({ users: data }: Props) {

    function handleDelete(id: number) {
        if (confirm('کاربر حذف شود؟')) router.delete(users.destroy(id).url);
    }

    return (
        <AuthenticatedLayout breadcrumbs={[{ title: 'مدیریت کاربران', href: users.index().url }]}>
            <Head title="کاربران" />
            <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-lg font-medium text-gray-800">کاربران ({data.total})</h2>
                    <Link href={users.create().url} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">+ کاربر جدید</Link>
                </div>
                ...
                <div className="bg-white rounded-lg shadow overflow-hidden">
                    <table className="w-full text-right text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {data.data.length === 0 ? (
                                <tr><td colSpan={6} className="text-center py-10 text-gray-400">کاربری وجود ندارد</td></tr>
                            ) : (
                                data.data.map(user => (
                                    <tr key={user.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-3 text-gray-600">{user.activePosition?.name ?? '---'}</td>
                                        <td className="px-4 py-3 text-gray-500">{user.activePosition?.department.name ?? '---'}</td>
                                        <td className="px-4 py-3">
                                            <span className={`text-xs px-2 py-1 rounded-full ${STATUS_COLOR[user.status]}`}>{STATUS_LABEL[user.status]}</span>
                                        </td>
                                        <td className="px-4 py-3">
                                            <div className="flex gap-3">
                                                <Link href={users.edit(user.id).url} className="text-yellow-600 hover:underline">ویرایش</Link>
